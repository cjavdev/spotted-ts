...
```sh
git clone git@github.com:cjavdev/spotted-ts.git
cd spotted-ts
./scripts/bootstrap
./scripts/build
```
...
> [!NOTE]
> Once this package is [published to npm](https://www.stainless.com/docs/guides/publish), this will become: `npx -y spotted-ts-mcp`
...
```json
{
  "mcpServers": {
    "spotted_ts_api": {
      "command": "node",
      "args": ["/path/to/local/spotted-ts/packages/mcp-server", "--client=claude", "--tools=dynamic"],
      "env": {
